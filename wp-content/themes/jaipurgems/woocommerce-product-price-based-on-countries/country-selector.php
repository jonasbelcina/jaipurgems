	...
	<form method="post" class="wcpbc-widget-country-selecting">		
		<select class="country" name="wcpbc-manual-country">
			<?php foreach ($countries as $key => $value) : ?>
				<option value="<?php echo $key?>" <?php echo selected($key, $selected_country ); ?> <?php if($_COOKIE['country_code'] != '' && strtoupper($_COOKIE['country_code']) == $key){ ?>selected<?php } ?> ><?php echo $value; ?></option>
			<?php endforeach; ?>
		</select>
	</form>			
